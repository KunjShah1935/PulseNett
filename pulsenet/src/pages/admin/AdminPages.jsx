      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {stats.map(s => (
          <Card key={s.title} className="p-4">
            <p className="text-xs text-slate-500 dark:text-slate-400">{s.title}</p>
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">{s.value}</h2>
          </Card>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
              ...
              {rooms.map(r => {
                ...
                return (
                  <div key={r.number} className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-mono w-16 text-slate-700 dark:text-slate-300">{r.number}</span>
                    <span className="text-xs w-20 text-slate-700 dark:text-slate-300">{r.type}</span>
        ...
        <Card title="Recent Admissions">
          <div className="p-4 space-y-3">
            {patients.map(p => (
              <div key={p._id} className="flex items-center gap-3 py-2 border-b border-slate-100 dark:border-slate-700">
                ...
                <div className="flex-1">
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">{p.patientId}</p>
          ...
          <Table headers={['ID', 'Name', 'Email', 'Role', 'Phone', 'Action']}>
            ...
            {users.map(user => (
              <tr key={user._id} className="border-b hover:bg-slate-50 transition">
                ...
                <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{user.customId}</td>
                <td className="px-4 py-3 font-medium text-slate-800 dark:text-slate-100">{user.fullname}</td>
                ...
                <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{user.phone}</td>
      ...
      <Card>
        <div className="overflow-x-auto">
          <Table headers={['Patient ID', 'Name', 'Age', 'Room', 'Doctor', 'Diagnosis', 'Status', 'Action']}>
            ...
            {forms.map(form => (
              <tr key={form._id} className="border-b border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition">

                {/* Patient ID */}
                <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{form.patientId}</td>

                {/* Name */}
                <td className="px-4 py-3 font-medium text-slate-800 dark:text-slate-100">{form.fullname}</td>

                {/* Age */}
                <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{form.age}</td>
                ...
                {/* Diagnosis */}
                <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{form.chiefComplaint}</td>
          ...
          return (
            <Card key={r.number} className="p-4">
              ...
              <div className="flex items-center justify-between mb-3">
                <div>
                  <span className="text-lg font-bold dark:text-blue-200" >{r.number}</span>
              ...
              <div className="flex justify-between text-xs text-slate-500">
                <span className="text-slate-500 dark:text-slate-400">{r.occupied} of {r.beds} beds occupied</span>
          ...
          <Card key={type} title={`${type} Charges`}>
            <div className="p-4 space-y-2">
              {charges.filter(c => c.type === type).map(c => (
                <div key={c._id} className="flex items-center justify-between py-2 border-b border-slate-100 dark:border-slate-700 last:border-0">
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          { label: "Total Patients Billed", value: summaries.length, color: "text-blue-600 dark:text-blue-400" },
          { label: "Revenue Collected", value: `₹${totalRevenue}`, color: "text-green-600 dark:text-green-400" },
          { label: "Pending Amount", value: `₹${totalPending}`, color: "text-amber-600 dark:text-amber-400" }
        ].map(s => (
          <Card key={s.label} className="p-4">
            <p className="text-xs text-slate-500 dark:text-slate-400">{s.label}</p>
            <h2 className={`text-xl font-bold mt-1 ${s.color}`}>{s.value}</h2>
          </Card>
        ))}
      </div>
      ...
      <div className="space-y-4">
        {summaries.map(summary => (
          <Card key={summary.patientId}>
            <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
              <div>
                <p className="font-semibold text-slate-800 dark:text-slate-100">{summary.patientName || summary.patientId}</p>
                <p className="text-xs text-slate-400 dark:text-slate-500">{summary.patientId} · Admitted: {new Date(summary.admissionDate).toLocaleDateString()} · {summary.days} day(s)</p>
              </div>
              <div className="flex items-center gap-3">
                ...
                <p className="font-bold text-slate-800 dark:text-white">₹{summary.totalAmount}</p>
              ...
              {newBill.items.map((item, i) => (
                <div key={i} className="flex gap-2 mb-2">
                  ...
                  >
                    ...
                    {charges.map(c => <option key={c._id} value={c.name}>{c.name} (₹{c.amount})</option>)}
                  ...
                  <div className="flex-1 pb-4">
                    <div className="flex items-start justify-between">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{log.user}</p>
                      </div>
                    </div>
                  </div>
                </div>
